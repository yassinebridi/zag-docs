...
If you're using React, you'll also need to install the `@ui-machines/react`
binding
...
By default, only one item may be expanded and each accordion item uses a unique
value, defined in the `getTriggerProps` and `getPanelProps`.
...
accordion item, trigger and content elements. When it closes, the
`data-expanded` attribute is removed. You can use this to style the expanded
states accordingly.

```css
/* Default styles for accordion trigger */
[data-part="trigger"] {
  background: white;
}
