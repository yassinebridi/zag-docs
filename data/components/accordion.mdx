...
Now you can use the accordion in your project!
...
To use the accordion component correctly, you'll need to understand its anatomy
and how we name its parts. Each part includes a `data-part` attribute to help
identify them in the DOM.
...
- `machine` — The state machine logic for the accordion widget as described in
...
> ensure that the accordion and its part has a unique identifier.
...
mode implicitly sets `collapsible` to `true` to also ensure that each accordion
can be expanded.
...
To set the value of the accordion(s) that should be opened initially, use the
`withContext(...)` pattern and pass the `value` attribute.
...
To allow an expanded accordion item to be collapsed again by clicking on it. Set
the context's `collapsible` property to `true`.
...
To disable a specific accordions, you can pass the `disabled: true` property to
the `getItemProps`, `getTriggerProps` and `getContentProps`.
...
- `getItemState()` — Function to check the state a accordion item is expanded.
  It returns an object containing `isOpen`, `isFocused`, `isDisabled`
...
[data-part="item"][data-expanded] {
  /* CSS for the item's expanded state */
}

[data-part="trigger"][data-expanded] {
  /* CSS for the trigger's expanded state */
}

[data-part="content"][data-expanded] {
  /* CSS for the content's expanded state */
}
```
...
[data-part="item"][data-focus] {
  /* CSS for the item's focus state */
}

[data-part="trigger"]:focus {
  /* CSS for the trigger's focus state */
}

[data-part="content"][data-focus] {
  /* CSS for the content's focus state */
}
```
