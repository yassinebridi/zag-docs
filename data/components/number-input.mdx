...
The NumberInput has controls for incrementing or decrementing numeric values.
...
- Handles floating point rounding errors when incrementing, decrementing, and
  snapping to step.
- Supports pressing and holding the stepper buttons to continuously increment or
  decrement.
- Support for stepper buttons and arrow keys to increment and decrement the
  value according to the step value.
...
The number input's connect exposes `inputProps`, `labelProps`,
`incrementButtonProps` and `decrementButtonProps`. Add these attributes to their
respective elements.
...
The number input machine supports the scrubber interaction pattern. The values
can changed by scrubbing on an arbitrary element with the `scrubberProps` spread
on it.
