...
The input fields allow one digit or letter at a time. When the digit or letter is entered, focus transfers to the next input in
...
To customize the default pin input placeholder (○), pass the `placeholder` prop and set it to your desired placeholder.
