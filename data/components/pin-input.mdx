...
To customize the default pin input placeholder (○), pass the `placeholder` prop
...
alphanumeric values, pass the `type` context property and set its value to
`alphanumeric`.
...
field receives a longer string by pasting into it, pin-input attempts to
