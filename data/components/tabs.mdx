...
Tabs are built using the `tablistProps`, `getTabProps`, `getTabPanelProps`
property and getter.
...
When a tab button is selected, a `data-selected` attribute is added to it. You
can use this to style the selected state.
...
clickable. To disable a tab, set the `disabled` property in the `getTabProps` to
`true`.
...
When the selected tab is changed, an `onChange` callback gets invoked with the
...
To set the tab to be selected initially, pass the `value` prop to the machine's
context.
...
To set the value of the selected tab programmatically, you can use the
`setValue` function provided by the connect function.

<CodeSnippet id="tabs/programmatic-control.mdx" />

## Vertical tab

To use vertical tabs, pass the `orientation` property to the context and set it
to `vertical`. This will ensure the up and down keys are used for keyboard
interactions.
