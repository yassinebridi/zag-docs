...
Tag inputs render tags inside an input, followed by an actual text input. Tags
are added when text is typed in the input field and the `Enter` or `Comma` key
is pressed. Throughout the interaction, DOM focus remains on the input element.
