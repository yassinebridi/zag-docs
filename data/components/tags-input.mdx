...
input element. Tag inputs render tags inside an input, followed by an actual
text input. Tags are added when text is typed in the input field and the `Enter`
...
- **Tag**: The tag data.
- **Tag Delete Button**: The button that deletes a tag.
- **Tag Input**: The editable field used to edit a tag.
- **Input**: The editable field used to enter a new tag.
...
tags input machine in your project 🔥 The tags input package exports two key
functions:
...
const [state, send] = useMachine(
  tagsInput.machine.withContext({
    validate(details) {
      return !details.values.includes(details.value)
    },
  }),
)
```
...
[data-part="root"][data-readonly] {
  /* styles for readonly tag */
}

[data-part="control"][data-readonly] {
  /* styles for readonly focus */
}
