...
There are four toast types that can be created with the toast machine. `info`,
...
> elapsed , we add `1000ms` delay before removing it from the DOM. You can use
...
> Remember every toast has a `1000ms` delay before it is removed from the DOM.
> We can leverage the `--remove-delay` css variable to apply this.
