...
In Chakra UI, we've experienced too many hiccups and bugs in the past related to
how we coordinate events, manage state, and side effects. Most these bugs are
associated with the orchestration within `useEffect`, `useMemo`, `useCallback`,
etc.

These issues were replicated in our Chakra UI Vue pursuit as well, and created a
maintenance hell for us. We're grateful this experience because it made us take
a step back to really define how we would like to build components in the
future.
...
- **Powered by state machines 🌳**: Zag is built on top of the latest ideas in
  Statecharts. We don't follow the SCMXL specifications, but we've created an
...
- **Write once, use everywhere 🦄**: The component interactions are modelled in
  a framework agnostic. We provide adapters for JSX frameworks so you can use it
  in React, Solid, or Vue 3.
...
- When you see someone using classic react, vue or solid to build an interactive
  UI component that exists in Zag, tell them to **"zag it!"**
...
past, I've been inspired by other meaningful projects and amazing people who
have inspire(d) us to keep improving our ideas.

Some of the projects I've been inspired by include:
...
- [Vue.js](https://vuejs.org/) and
  [Lit](https://lit-element.polymer-project.org/) for inspiring new patterns in
  the machine (`computed` and `watch`)
