...
state (or data) within the machine. To extend this initial value of this
context, we expose the `.withContext(...)` method.
...
`api`. You can pass the context value to the `useMachine` hook directly.
...
> This only works for context updates, not internal state updates and is not the
